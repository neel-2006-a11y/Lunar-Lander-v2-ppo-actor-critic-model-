...
class PPOAgent:
    ...
    def __init__(self, env_name):
        ...
        self.env = gym.make(env_name, max_episode_steps=800)
        self.action_size = self.env.action_space.n
        self.state_size = self.env.observation_space.shape
        self.EPISODES = 40000 # total episodes to train through all environments
        self.episode = 31800 # used to track the episodes total count of episodes played through all thread environments
        self.lr = 0.00025
    ...
    def run(self): # train only when episode is finished
        self.load_model("models_raw/actor_ep31800.h5", "models_raw/critic_ep31800.h5")
        scores = []
        ...
        while True:
            ...
            while not done:
                ...
                # angle shaping
                reward-=0.0*abs(angle)

                # land
                if (terminated and y_pos<0.1):
                    if(abs(x_pos)>0.6):
                        reward-=30
                    if abs(y_vel)<0.5:
                        reward+=30
                    reward-=abs(y_vel)*30
                    reward-=20*abs(angle)
                    reward-=20*abs(ang_vel)
                    reward-=50*abs(x_pos)
                    if(y_pos>0.3):
                        reward-=50*abs(y_pos)

                if(truncated):
                    if(y_pos<0.3 and abs(x_pos)<0.35):
                        reward+=(0.3-y_pos)*30
                ...
                if done:
                    ...
                    if (self.episode) % 200 == 0:
                        self.Actor.Actor.save(f"models_raw/actor_ep{self.episode}.h5")
                        self.Critic.Critic.save(f"models_raw/critic_ep{self.episode}.h5")
    ...
    def test(self, test_episodes = 20):
        self.load_model("models_raw/actor_ep30600.h5", "models_raw/critic_ep30600.h5")
