...
class PPOAgent:
    ...
    def __init__(self, env_name):
        ...
        self.episode = 7600 # used to track the episodes total count of episodes played through all thread environments
        self.lr = 0.00025
    ...
    def replay(self, states, actions, rewards, predictions, dones, next_states):
        ...
        advantages, target = self.get_gaes(rewards, dones, np.squeeze(values), np.squeeze(next_values), normalize=False)
        target = np.hstack([target,values])
    ...
    def run(self): # train only when episode is finished
        ...
        self.load_model("models5/actor_ep7600.h5", "models5/critic_ep7600.h5")
        scores = []
        ...
        while True:
            ...
            while not done:
                ...
                if truncated and not terminated:
                    print("/////////truncated")
                    if(y_pos<0.15):
                        reward+=500*(0.15-y_pos)
                    if(y_pos>1):
                        reward-=500
                    if np.sqrt(x_pos**2+y_pos**2)>0.3:
                        reward-=250*(np.sqrt(x_pos**2+y_pos**2)-0.3)
                    # if np.sqrt(x_pos**2+y_pos**2)<0.1:
                    #     reward+=(0.1-y_pos)*500

                    # reward-=100*(np.sqrt(x_pos**2+y_pos**2))
                    print("reward after truncate penalty:", reward)

                # Centering
                reward -= (abs(x_pos))*4.0 # attract toward center
                reward += (2-abs(x_pos))*0.3

                # Velocity shaping
                if y_pos>0.1: # after this, the landing penalty takes control
                    reward += (2-abs(y_vel))*1
                    reward-=abs(y_vel)*10
                if abs(y_vel)>0.5 and y_pos<0.5:
                    reward -= 0.7

                # Altitute shaping
                # reward += 0.1 * (prev_y - y_pos)   # reward for going downward
                # prev_y = y_pos
                if(y_pos>0.8): # go down at high_altitude
                    reward-=0.3
                if(y_pos<0.15): # land at low altitude
                    reward-=1*y_pos

                # Angle shaping
                reward-=7*(abs(angle))
                reward+=1*(1-abs(angle))
                ...
                if terminated:
                    ...
                    #     reward-=500
                    if (abs(x_pos<0.3)):
                        if ((left_leg or right_leg) and abs(y_vel)<0.5):
                            reward += 400
                            if abs(y_vel) < 0.1 and abs(x_vel) < 0.2: # safe velocity
                                reward += 400
                            if abs(x_pos) < 0.2 and abs(angle) < 0.1:
                                reward += 250
                        if ((left_leg and right_leg) and abs(y_vel)<0.5):
                            reward += 400
                    # else:
                    #     if (abs(x_pos<0.7)):
                    #         if ((left_leg or right_leg) and abs(y_vel)<0.5):
                    #             reward += 500*(0.7-abs(x_pos))
                    #             if abs(y_vel) < 0.3 and abs(x_vel) < 0.5: # safe velocity
                    #                 reward += 375*(0.7-abs(x_pos))
                    #             if abs(x_pos) < 0.2 and abs(angle) < 0.1:
                    #                 reward += 250*(0.7-abs(x_pos))
                    #         if ((left_leg and right_leg) and abs(y_vel)<0.5):
                    #             reward += 1000*(0.7-abs(x_pos))
                    #distance penalty
                    if np.sqrt(x_pos**2+y_pos**2)>0.3:
                        reward-=150*(np.sqrt(x_pos**2+y_pos**2)-0.3)
                        reward-=50*x_pos
                    # velocity penalty
                        reward-=150*abs(y_vel)
                    # angle penalty
                        reward-=150*abs(angle)
                        reward-=150*abs(ang_vel)

                # Time pressure
                reward -= 0.4   # every step costs time
                # Angle stability
                reward -= abs(angle) * 0.8
                reward -= abs(ang_vel) * 0.5
                ...
                if done:
                    ...
                    if (self.episode) % 250 == 0:
                        self.Actor.Actor.save(f"models5/actor_ep{self.episode}.h5")
                        self.Critic.Critic.save(f"models5/critic_ep{self.episode}.h5")
    ...
    def test(self, test_episodes = 20):
        # self.load_model("models2/actor_avg_above_0.h5", "models2/critic_avg_above_0.h5")
        self.load_model("models5/actor_ep3400.h5", "models5/critic_ep3400.h5")
